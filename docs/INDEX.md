...
총 **78개**. 설명은 원문 그대로 둔다 - 여기서 훑어보고 필요한 것만 여는 용도라 요약을 줄이면 그 값이 사라진다.

### 최근 26개

| 날짜 | 문서 | 내용 |
| --- | --- | --- |
| 2026-09-03 | [skill-camera-superarmor-stamina](dev-logs/2026-09-03-skill-camera-superarmor-stamina.md) | ★★**SB 는 스킬에서 시점을 안 뺏는다** — Eve 511파일에 시점고정·입력차단 키가 **없다**(CamShake 216 · ControlCamLagSpeed 55 · CamAnim 62 뿐). 반격은 **FOV -70 펀치 + 랙속도 19→1.0**. → 우리도 **B안(카메라 애니메이션 = 오프셋 가산)** 채택, A안(레벨 시퀀스)은 처형급 전용. ★`ModifyCamera` 가 스프링암·락온 계산이 **끝난 `InOutPOV` 위에서** 돈다(`:445-452`) → 시점 유지 · 락온 공존. `Scale` 이 **위치·회전·FOV 에 모두** 곱해져 시퀀스 1개로 3단계(`:434`). 🔴**`TemplateSequence` 플러그인 수동 활성 필요**(EnabledByDefault false) · 모디파이어는 `GameplayCameras` 아닌 **`EngineCameras`** · 생성 메뉴는 Animation 아닌 **Cinematics**. 🔴🔴**FOV 는 절대값이 아니라 "차이"** — `ResetDefaultValues` 가 **매 프레임 현재 게임 FOV 에서 기준 초점거리를 역산**한다 → 게임 FOV 가 낮으면 **같은 값이 반대로(넓어짐) 작동**. 락온 중엔 `DefaultFOV` 75 고정이라 그때 테스트할 것. ★★**시네카메라 센서폭 = 23.76mm**(36mm 로 오판해 환산표를 통째로 틀렸다) → **15.5mm 가 75도 기준선**, 18mm = -8.1도. 초점거리를 애니하면 `RecalcDerivedData` 가 FOV 로 변환(별도 트랙 불필요). ★차지 줌 = **`bLoop=true` + `StopAllCameraAnimationsOf`** — 시퀀스 에셋만으로 전 인스턴스를 멈춰 **핸들 저장이 불필요**(노티·GCN 은 상태를 못 든다). 키 2개 **동값**이 "값을 붙들어두는 장치"(Skill_02 는 0→값→0 · bLoop=false 로 정반대). ⚠️`bImmediate=true` 면 단계 전환 때 화면이 튄다 → `false` 크로스블렌드. ⚠️섹션 경계 노티는 점프 진입 시 놓칠 수 있어 **2프레임 안쪽**(1.033). 📌시퀀서에 **액터를 넣지 않는다**(팩토리가 `ACineCameraActor` 를 바인딩 고정) · 런타임은 대역(`CameraStandIn`)에 평가 · **프리뷰로는 판단 불가, PIE 가 유일**. ★★**슈퍼아머 태그는 있었는데 배선이 반쪽이었다** — `State.Combat.SuperArmor` 를 `EnemyHitReact.cpp:21` 이 차단하는데 **부여하는 GA 가 17개 중 0건**. 스킬이 끊기던 건 GAS 취소가 아니라 **리액션 몽타주가 같은 슬롯을 덮어서**(`GA_HitReact.CancelAbilitiesWithTag` 는 비어 있다) → `GA_HitReact` Blocked + 스킬 4개 Owned 로 **BP 5개·코드 0줄** 해결. ★★**스태미나 = 시간이 주는 자원 → 때려서 버는 자원** — `GE_StaminaRegen` 을 StartupEffects 에서 제거하고 **데미지 게이트웨이(`PostGameplayEffectExecute`)에 `GainAttackerStamina` 신설**(§1-2 대로 한 곳만 고쳐 평타20·스킬4·총·범위 전부 커버). **평타 화이트리스트**(`Ability.Player.Light/Heavy`)라 스킬이 늘어도 자동 제외 + 적은 `Ability.Player.*` 를 안 써 **소유자 검사까지 겸한다**. 🔴🔴**처음엔 안 됐다 — `GetAbility()` 가 항상 nullptr**: `UGameplayAbility::MakeEffectContext` 는 `SetAbility(this)` 를 하지만 **`ASC::MakeEffectContext` 는 안 한다**(`KDAbilityStatics.cpp:103` 이 ASC 쪽). **리젠이 가려주고 있었을 뿐 처음부터 회복이 안 되고 있었다.** → `ApplyDamageEffect` 에 `SourceAbility` 인자 추가 + 호출 3곳 `this`. ⚠️`GetAbility()` 는 복제 X(멀티 시 GE AssetTag 로 교체). ⚠️`UKDStaminaComponent` 는 **실재하지 않는다**(`KDGameplayTags.h:95` 주석이 낡음) |
| 2026-09-03 | [skill-charge-vfx](dev-logs/2026-09-03-skill-charge-vfx.md) | ★★**홀드 중에는 AnimNotify 가 전부 죽는다** — `SkillCharge.cpp:33-35` 가 진입 섹션 끝(0.167s)에서 `Montage_Pause` 를 걸어 홀드 구간엔 몽타주 시간이 안 흐른다. 노티를 꽂아도 **에러도 로그도 없이 하나도 안 터진다**(Skill_01·02·04 는 해당 없음). → 연출 신호를 **GA 타이머**로 옮기고 §1-1 에 따라 **GC 경유**. ★큐는 **태그 1개 + `RawMagnitude`**(`GameplayEffectTypes.h:863`) — `BP_GCN_EnemyTelegraph` 는 자식 태그 3개로 갈랐지만 여긴 **단계 = `ChargeSteps` 배열 인덱스**라 매그니튜드가 맞다(단계가 늘어도 태그를 안 만든다). ★**단일 진실 유지** — `MinHoldTime` 하나 고치면 애니 분기와 이펙트 타이밍이 같이 바뀐다. ★**`FTimerDelegate` 가 필요한 이유** = `SetTimer` 기본형은 인자 없는 함수만 건다 → `CreateUObject` 페이로드로 인덱스를 싸둔다(비동적이라 `UFUNCTION` 불필요). ⚠️`SetTimer(0)` 은 조용히 무시되므로 0단계는 즉시 호출. ★`ClearStepTimers()` **3곳** — `OnActivated`(InstancedPerActor 잔류) · `ReleaseCharge`(**0.5초에 떼면 0.8초 타이머가 살아 있다**) · `OnCleanup`. **타이머는 처음에 전부 예약해 각자 독립 발화** — "단계마다 이전 것 끄고 반복"이 아니다. ★`K2_` = Kismet 2 접두사일 뿐 평범한 C++ 함수(`GameplayAbility.cpp:1710` 이 `AbilityLevel` 채우고 ASC 호출 — 직접 부르는 것보다 낫다). ★★**AuraFX 계열은 파라미터로 형태가 갈린다** — **`SwordLength` = 검용 / `Sphere Radius` = 몸용**(이름만 보면 다 같은 "Aura"). `Scifi`(에미터 2)는 체급 미달, `ChargingBow`(10)는 **`userParameterCount 0` 이라 런타임 틴트 불가** → 승환 제안으로 **NS 에 유저 파라미터를 직접 노출**(복제 3개 폐기). ★**답안이 프로젝트 안에 있었다** — `NS_SB_Telegraph_Red` 가 같은 10에미터 계열인데 `User.Color` 가 이미 뚫려 있다. ⚠️`Link Inputs` 목록에 뜨려면 **타입이 맞아야**(Color 입력=LinearColor, Vector 로 만들면 안 보임) · **에미터마다 내부 색이 달라 전부 링크하면 한 색으로 뭉개진다**. 🔴색 변경 폭 제한(머티리얼이 지배) — 현행 유지. 🔴GC BP 는 **`DefaultGame.ini:19-20` 스캔 경로 안**이어야 하고 밖이면 조용히 안 잡힌다. 🔴`Set Niagara Variable **By String**` 은 5.3 deprecated · `InVariableName` 비면 무동작(`User.` 접두사 뺀다) · `Select` 는 **출력 먼저** 연결해야 Wildcard 가 굳는다. ★`To LinearColor (Vector)` 는 **알파를 1.0 으로 채운다**(`Color.cpp:45`) · `OnExecute` 반환값은 **버려진다**(`GameplayCueNotify_Actor.cpp:286`). 🔴**`ChargeSteps[0].MinHoldTime` 은 반드시 0.0** — 크면 조기 릴리즈 시 `Step=nullptr` 이라 섹션 링크를 안 바꿔 전 구간이 재생된다. 값 = 0/3/5 · Max 6(⚠️5초 홀드 = 5초 정지 화면, 현행 유지). 📌Skill_01 — **방사형이 이미 있었다**(`PowerUp_01` 이 `SM_FX_Circle_01`+`Shockwave_Air`, `NS_PerfectDodge` 와 같은 조합) → `NS_SB_Telegraph_Red` 복제 교체. ⚠️**`ENiagaraRingDiscMode` 는 방사형 신호가 아니다**(거의 모든 NS 에 붙은 Shape Location enum) — 판별은 머티리얼·메시로. ⚠️Vefects Shockwave 는 `User.Color` 가 **컬러커브**라 런타임 색 변경 불가(그래서 18색을 구워 놓음) → 복사본 95개 삭제. 📌**Skill_02 땅 찍기 = VFX·슬로우·쉐이크를 60프레임에 겹쳤다**(`NS_SB_Free_Magic_Circle2` + `WindupSlow` 0.2 + `CameraShake_SB_XL`). ★★**슬로우와 NS 길이는 묶여 있다** — `WindupSlow` 는 `Montage_SetPlayRate` 로 **몽타주만** 늦추고(`CustomTimeDilation` 아님) **나이아가라·쉐이크는 실제 시간으로 흐른다** → `필요 NS 길이 = 구간 애니 길이 ÷ SlowRate`(0.242÷0.2 = 1.21s = 60fps 73프레임). 🔴**`SlowRate` 를 먼저 확정하고 NS 를 자른다**(반대면 매번 다시 손봐야 함). 📌사라지는 시각 = `Loop Duration` + `Lifetime`(스폰이 멈춰도 마지막 입자가 더 산다) · **수명은 유저 파라미터로 안 나와 있어 MCP·파이썬으로 못 읽는다 = PIE 눈 검증만 가능**. ★`UKDAnimNotifyState_WindupSlow` 는 **적 예고용**인데 플레이어에 재사용 — `NotifyEnd :47` 이 **재생률이 "내가 건 값"일 때만 복구**해서 경직 몽타주 속도를 안 덮는다. ⚠️**`SlowRate` 하한 0.05** — 0 이면 몽타주 시간이 안 흘러 `NotifyEnd` 가 영영 안 온다(자기가 건 정지를 자기가 못 푼다) → **완전 정지는 `UKDHitStopComponent`**(`CustomTimeDilation=0` + `ResumeTimer`) 담당. ⚠️`GetEffectiveSlowRate :11` 이 `Cast<AKDEnemyBaseCharacter>` 라 **플레이어는 배수가 안 먹는다**(동작은 정상) · 루트모션 전진 구간에 걸치면 이동 거리가 준다. ★카메라 쉐이크는 **`AN_CameraShake` 노티 + `CameraShake_SB_XL` 로 끝**(코드·GC 0) — GC 경로(`UKDAnimNotify_PlayerCue` + `LCS_*` 5종)도 있지만 **어떤 세기가 맞는지 모르는 채 배선부터 늘리지 않으려고** 미룸. 📌`NS_SB_Free_Magic_Circle2` 는 **유저 파라미터 19개**(에미터 5개 각각 Color/Scale/Velocity/Shape + Scale_All)로 지금까지 본 것 중 가장 잘 열려 있다 |
| 2026-09-02 | [skill-slot-ui](dev-logs/2026-09-02-skill-slot-ui.md) | ★우하단 스킬 슬롯 UI — **코드 0줄**(텍스처·머티리얼·위젯만). ★★**SB 를 안 보고 설계했다가 두 번 뒤집혔다** — `CURRENT.md` 의 *"SB식 슬롯"* 은 할 일 문구지 실측이 아니었고 메모리 SB UI 자료에 **스킬 슬롯 항목이 아예 없었다**. 승환이 *"SB 스타일 맞재?"* 물어 찾아보니 **바탕화면에 스샷이 있었다**(`SB 스샷/UI/ItemAbility Cross Menu.jpg`) → 실물 = **원형 슬롯 · 십자 배치 · 라인아트 픽토그램**(KD 1차안 = 사각·가로나열·곡선실루엣). ⚠️초확대에서 **한 번 더 정정** — 마커는 흰 원+검정이 아니라 **어두운 원+흰 기호**(`L1` 하나만 반전인데 그걸 4개에 옮긴 오독). ★승환 관측이 규칙이었다 — **버튼 마커는 항상 중앙 향한 변에 붙는다**. 십자 배치는 가져오되 게임패드 대응(△□○✕)은 버리고 숫자키 1~4 를 마커 자리에. ★★**알파 없는 AI 이미지를 UI 로** — 생성 모델에 투명 배경 파라미터가 없다 → `Domain UI + Blend Additive` 면 **검정이 저절로 투명**(노드 1개, Opacity 배선 X). ⚠️전제 = 배경이 순검정 — **GPT (0,0,0)✅ / Recraft (21,21,21)❌** (`background_color=#000000` 을 줘도 무시, 2회 재현). 🔴**MCP `manage_asset --import` 는 프로젝트 밖 파일 거부**(SECURITY_VIOLATION) — 프로젝트에 파일 안 만드는 룰과 정면 충돌 → **`unreal.AssetImportTask` 파이썬으로 우회**(사운드·메시도 같은 길). UI 텍스처 설정 4개 필수 + **파이썬 enum 이름이 표시명과 다르다**(`UserInterface2D` = `TC_EDITOR_ICON`). ★머티리얼 **텍스처 0장으로 원을 그린다**(UV 중심거리 + SmoothStep) — 파라미터 5개로 인스턴스 3개(슬롯·배지·덮개). 🔴쿨다운 덮개가 **사각형이라 원 밖으로 삐져나왔다** → 같은 머티리얼 인스턴스로 바꾸니 반지름이 겹쳐 해결(**겹쳐 쌓는 UI 는 같은 머티리얼에서 파생시킨다**). ★★**쿨다운 원형 스윕** — `Arctangent2(dx, -dy)` 로 **12시 시작 시계방향**(UV 는 아래가 +Y 라 `atan2(dy,dx)` 면 3시 시작), `Frac` 정규화 후 `Progress` 로 자른다. **`UMovieSceneWidgetMaterialTrack` 이 실재** → 위젯 애니메이션이 머티리얼 파라미터를 직접 움직인다(**BP 노드 0개**). ⚠️`Image_Dim` 의 `Render Opacity` 는 **1.0** — 안 보이는 건 `Progress=0` 담당(두 값 역할 혼동 주의). 📌UMG 함정 4개 — `SizeBox` Width/Height 는 **각각 체크박스**(editcondition) · 디자이너 **Fill Screen 이 세로로 늘려 그린다**(→Desired) · **`Is Variable` 기본 꺼짐**(MCP 로 CDO 노출 여부로 확인 가능) · `HorizontalBox` 슬롯 Fill 이면 Override 무시. 📌`Set Render Opacity`(float, 전 위젯 공통) 가 `SetColorAndOpacity`(Image=LinearColor / TextBlock=**SlateColor**) 보다 낫다 |
| 2026-09-02 | [area-blast-ga](dev-logs/2026-09-02-area-blast-ga.md) | ★범위 판정 GA `UKDGameplayAbility_AreaBlast` — 스킬2 땅 찍기. `GA_ShotBlast` 와 같은 층위의 독립 GA(스킬 GA 는 이 일을 모른다). 부모를 안 건드린 이유 = `PlayerMelee` 는 무기 소켓 스윕 전용이라 **콤보 GA 20개가 안 쓰는 코드를 물려받는다**. ★★**노티를 새로 만들 뻔했다** — 범용 BP `AN_SendGameplayEvent` 가 이미 있었고 `KDGameplayTags.h:119` **주석이 그 이름을 부르고 있었다**. 원인 = 기존 클래스 검색을 `Source/` 에만 하고 `Content/` 에 안 했다(`Event_Rush_Warp` "C++ 발신처 0건" 이 곧 "BP 가 쏜다" 신호였다). ★전용/범용을 가르는 건 **`Payload.OptionalObject`** — 캐스팅할 타입이 필요해서 전용 클래스가 존재한다(ShotBlast 는 오버라이드 7개, AreaBlast 는 0개). ★락온 `GatherCandidates` 를 안 쓴 이유 = private·단일반환 + **"락온 받을 자격"(CanBeTargeted)이 데미지 판정에 섞인다** + 그 컴포넌트가 이미 552줄. ★부채꼴 = **`각도 > HalfAngle` == `Dot < cos(HalfAngle)`**(cos 가 감소함수라 부등호가 뒤집힌다, `acos` 는 느려서 안 씀). ⚠️`HalfAngle` 은 **반각**(90 → 부채꼴 180도) · `GetSafeNormal2D` 가 Z 를 버려 높이는 각도에 안 낀다. **LoS 미적용**(승환 결정 — 범위 공격은 엄폐 무시). ★방사형 넉백·경직은 **코드 0줄** — `KnockbackComponent.cpp:46` 이 공격자 반대 방향으로 밀고, 경직은 `InstigatorTags.HasTag` 라 AssetTag 가 곧 키(⚠️적 DA 에 키가 없으면 **에러 없이 Poise 0**). ★★빌드 함정 2개 — **`TObjectPtr` → `TWeakObjectPtr` 대입 C2679**(템플릿 추론이 사용자 정의 변환을 안 쓴다 → `.Get()`), 이어진 **C2440 은 원인이 달랐다**(`.h` 전방선언 + `.cpp` include 누락 — **둘은 항상 짝**). 🔴**`Build.bat` 은 컴파일 에러에도 exit 0** → `Result: Failed` 문자열로 판정할 것. ★스킬 몽타주 4개 연출 배선 44개 — 콤보 20개 표준 트랙 실측 후 `WeaponAttach`/`Sound_Swing`/`WeaponTrail`/`Sound_Shot`/`Muzzle` 추가, `CancelWindow`·`MovementCancel`(캔슬 없음)·`Warp`(ApproachWarpName=None) 제외. **총격 3노티 세트가 깨져 있었다**(Skill_01·03 이 `Shot` 만 = 총성·섬광 X) / 검 쪽은 4개 전부 궤적·소리 0개. ★★**`Muzzle` 이 `WeaponTrail` 클래스인 이유 = GC 가 총 메시를 못 가리킨다**(`GameplayCueNotifyTypes.cpp:337` 이 캐릭터 메시만 본다). ⚠️클래스 기본값이 `Sword` 라 **세 값이 한 세트**(ns·socket·`weapon_mesh_component_tag`) — 태그를 빠뜨려 9개를 사후 수정했다. ⚠️`Shot` 을 옮겨도 `Muzzle` 은 안 따라온다(타임라인에 검 궤적과 같은 이름) → **트랙 통째 재생성이 멱등 해법**. 📌MCP — `get_animation_notify_track_names` / `add_animation_notify_state_event` 계열, `LinkValue` 는 **앞이 끝·뒤가 시작**, `TrackIndex` 는 export 전문에. 🔴**24개 일괄 투입 중 에디터 크래시**(파일럿만 생존) → 한 개씩 넣고 즉시 저장으로 전환, 원인 미확정 |
| 2026-09-02 | [skill-charge-ga](dev-logs/2026-09-02-skill-charge-ga.md) | ★캐릭터 스킬 GA — 단발 3종 + **차지형 1종**. 신규 클래스 2쌍 + 태그 6개. ★**`SetByCaller` GE 는 Cost 로 못 쓴다** — `CheckCost` 가 CDO 를 그대로 넘겨 매그니튜드가 0(`GameplayEffect.cpp:5181`) → 두 함수 오버라이드로 우회(`Super` 도 안 부른다, 경고 로그가 매번 뜬다). ★차지 = **몽타주 1개 + 섹션 5개**, 몽타주를 안 나눈 이유 = 부모의 `OnMontageCompleted` 가 private·non-virtual 이라 이어 돌리려면 **콤보 GA 20개가 보는 부모**를 고쳐야 한다 + 초당 600cm 전진 중이라 이음새 블렌드가 튄다. `SectionName` 은 "다음 섹션"이 아니라 **"마지막으로 재생할 섹션"**(그 출구만 End 로 교체). ★컷 지점은 **포즈 전수 스캔**으로 결정 — 안무가 연결점 11.3도 vs 중간 컷 최선 21.6도, 최악 60도(골반 137도). **인접 섹션 전환은 안 튀고 건너뛰는 점프만 튄다.** ★루프 루트모션 = **100cm 씩 5번 계단식**(5프레임에 100cm = 초당 1200cm), 판정 노티가 각 버스트에 하나씩 → 차지 단계가 1m 단위로 떨어짐. ⚠️최소 이동 6m(Start 97 + End 409 는 항상 붙는다). ★**GA 에는 Tick 이 없다** → 타이머 3개(정지·상한·복구). 부모 안전망을 끄면 **대신할 것이 필요**하다(필요 없어서 끄는 게 아니라 길이가 안 맞아서 갈아끼움). `Montage_Pause` 는 프로젝트 유일 사용처 — 히트스톱은 `CustomTimeDilation` 방식(`PROJECT_OVERVIEW §2-5` stale 정정). ★키 뗌은 **GameplayEvent 로** — `WaitInputRelease` 는 입력 ID 바인딩이 0건이라 안 돈다, 캔슬은 `End` 마무리 타격이 날아가서 불가. 입력 배선은 **기존 구조(Controller→Character→InputComponent)로 통일**(`Handle_Move` 가 예외지 본이 아니다). 📌조회법 2개 — 섹션 시각은 파이썬 X·MCP `inspect` O(`LinkValue`), GE 태그는 `GEComponents` 의 `InheritableGrantedTagsContainer`(표시명 "Add Tags") |
...
| 2026-08-31 | [parry-effect-sb-style](dev-logs/2026-08-31-parry-effect-sb-style.md) | ★★`GCN_PerfectParry` 가 **길동 창 소켓 `Spear_Tip`** 에 이펙트를 붙이고 있었다(Content 전수 스캔 1건). 우회한 이유 = **위치를 받을 통로가 없었다** — `KDCombatAttributeSet` 퍼펙트 패링 분기가 `ContextHandle` 을 안 실었다(적 방어형 패링 `:151` 은 이미 싣고 있었다). 코드 **한 줄**로 통로를 열고 `GetHitResult(Parameters) → ImpactPoint` 로 SB식 접촉점 스폰. ★★**같은 큐를 두 곳에서 쏘고 있었다** — `GA_Parry` 와 `GA_PerfectParryReaction` 이 둘 다 `PerfectParry.SlowMo` 를 발신해 **액터 2개·MPC Timeline 2개·슬로모 2회**. 08-30 슬로모 Subsystem 이 배율을 같게 만들어 **증상을 덮고 있었다**. `GA_Parry` 쪽만 남김(`Break Event Data` 가 거기 있고 좌/우 리액션도 거기서 고른다 — `HitAngle` 로 `AM_SB_Parry_L/R` 분기, 패링 성공 애니는 **있었다**). ★SB 실측 = `Result_Parryed` 에만 CamShake, `Result_Guard`·`Guard_Break`·`Parry` 는 Anim+Sound 만 → **B4 규모 절반으로 축소**. `LCS_ParryClash`(고아 `LCS_PlayerHitConfirm` 개명 + `CS_PC_DefaltParry_01` 이식: Dur 0.35/In 0/Out 0.15/Loc 3축 amp3 freq40). ⚠️적이 쏘는 큐라 `Play In World` 를 켜야 뜬다(`GameplayCueNotifyTypes.cpp:652` 가 `TargetActor` 하드코딩 — 적엔 PC 가 없다). ★조사 교훈 = **`OnExecute` 는 함수 그래프**라 `get_graph_details`(EventGraph)에 안 나온다. `get_blueprint` 의 `functions` 를 먼저 볼 것 |
