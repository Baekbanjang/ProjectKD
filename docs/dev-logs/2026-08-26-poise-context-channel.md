...
✅ **통로 관통 확인 완료.** `DA_ComboTree` 노드 하나에 `PoiseMultiplier = 3.0` 을 넣고 그 타의 Poise 칸이 3배로 깎이는 것을 확인했다. 확인 후 0 으로 원복(`git checkout`, 에디터 종료 상태에서).

값이 전부 1.0 이면 먹는지 안 먹는지 구분이 안 되므로, **이 확인 없이 C1 로 넘어가면 안 됐다.**
