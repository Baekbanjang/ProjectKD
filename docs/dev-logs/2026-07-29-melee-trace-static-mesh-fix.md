...
23~78%. 비교로 **1~3타의 실측 캔슬은 35~57%에 몰려 있다**(중앙값 46%, 포즈 대조로 뽑은 진짜 값). 피니셔는 후딜이 더 길어야 리스크가 성립하므로 55~65%대가 출발점이지만, **결국 "막타 치고 얼마나 빨리 도망칠 수 있게 할 거냐"는 손맛 판단**이라 PIE에서 정한다.

**결정(2026-07-29)**: `01_04` 27→**64**(53%), `02_04` 44→**64**(54%). 나머지 3개는 유지. **MCP 연결이 끊겨 미반영 상태** — 다음 세션에서 넣는다.

## ★ `MovementCancel` = `CancelWindow` + 3프레임 (2026-07-29 결정)

두 노티를 **같은 프레임에 두면 안 된다.** 서로 다른 코드 경로에서 **똑같이 `CancelAbilities`를 호출**하기 때문이다.

| 경로 | 위치 | 발동 조건 |
|---|---|---|
| `CanCancel` | `KDPlayerCharacter.cpp:88-101` (Tick) | 버퍼에 공격 입력이 **있을 때만** |
| `MovementCanCancel` | `KDPlayerController.cpp:94-101` (`Handle_Move`) | 이동 스틱 홀드 중이면 **무조건** |

두 번째가 무조건인 이유는 바인딩이 `ETriggerEvent::Triggered`라서다 — **스틱을 잡고 있는 동안 매 프레임 호출된다.** 새로 입력할 필요가 없다.

같은 프레임이면 **액터 틱 순서**(Controller 입력 처리 vs Character Tick)가 승자를 정하는데, UE는 액터 간 틱 순서를 보장하지 않는다. 같은 조작이 다른 결과를 내는 **재현 불가능한 버그**가 된다.

> 실제 결과 차이는 작다. `Tick:396`의 조건이 `(!bAttacking && !bDodging) || bCanCancel`이라, 이동이 먼저 GA를 죽여도 `bAttacking`이 false가 되어 같은 프레임에 버퍼된 공격이 나간다. **얻는 것은 결정성**이며, 비용이 50ms(체감 불가)라 안 할 이유가 없다.

**3프레임인 근거**

| | 값 | 근거 |
|---|---|---|
| 하한 | **3f (50ms)** | 30fps의 1틱 = 33ms. 그보다 커야 두 노티가 다른 틱에 걸린다 |
| 상한 | 6f (100ms) | 이 이상은 이동 캔슬 지연이 체감된다 |
| 참고 | InputBuffer 0.2s = 12f | 3f는 그 1/4이라 버퍼 소비에 지장 없다 |

60fps만 보면 1프레임으로도 레이스는 해소되지만, 프레임이 떨어지면 두 노티가 한 틱에 같이 시작해 다시 경쟁한다. 3은 그 여유분이다.

> ⚠️ **나중에 "왜 3프레임 어긋나 있지?" 하고 맞추지 말 것.** 의도된 값이다.

### 남은 부작용 (지금 스코프 밖)

`Handle_Move`가 `Triggered`인 이상, **이동을 잡은 채 공격하면 회복 모션이 매번 캔슬 시점에 잘린다.** 플레이어가 "캔슬하겠다"고 선택한 게 아니라 스틱을 쥐고 있어서 잘리는 것이다. 3프레임으로는 못 막고, 막으려면 방향 전환이나 재입력을 요구하도록 바꿔야 한다. "이동 중 콤보가 자꾸 끊긴다"는 체감이 오면 여기가 원인이다.
