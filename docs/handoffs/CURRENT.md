...
```
적 상태 바 표시 조건  락온이 유일한 스위치 (`OnTargeted_Implementation` -> `SetVisibility`).
                    승환 요청 = **전투 진입 시 · 피격 시에도 자동 표시.** 설계 = 이유 3개를 OR 로 합칠 것
                    (각자 SetVisibility 하면 락온 해제가 나머지까지 끈다). 피격은 타이머로 상태화
                    적 쪽 "전투" 신호 = AIController 블랙보드 `TargetActor` (bInCombat 은 AnimInstance 지역 변수)
