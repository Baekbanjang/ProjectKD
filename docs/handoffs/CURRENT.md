...
**4단계 — 막기의 대가 (설계 변경. 조건부)**
```
4-1  Shield 를 막기 자원으로 전환
     지금은 막기와 무관하게 상시 40% 흡수 (KDCombatAttributeSet.cpp:150)
     -> 바꾸면 적 6종 Shield 값 의미가 통째로 바뀐다 (플레이어 50 / 패리밴딧 30 / 엘리트 60)
4-2  가드 붕괴   Shield 0 -> 무방비. 애니 AM_SB_Block_Hit_Break 가 이미 있다
     ⚠️ 그게 실제로 쓰이는지는 아직 확인 안 됨
```
⚠️ **2-3 은 4단계와 짝이다.** 막기가 공짜인 채로 Poise 만 못 깎게 하면 막기가 더 좋아진다.
4단계를 안 할 거면 2-3 도 보류한다.
📌 4단계를 안 해도 게임은 돈다. 다만 **막기 남발이 최적해로 남는다.** 1~3 만으로 충분할 수도 있다.

**5단계 — 적 전조(읽기).** 가장 비싸다(적 애니·큐·AI). **1단계에서 "못 읽겠다" 가 나올 때만.**

```
0  Timeline 길이 확인    승환 · 5분
1  값 맞추기 + PIE 진단   에셋만 · 반나절     <- 여기서 방향이 갈린다
2  반격 사슬             코드+에셋 · 가장 큼
3  연출 이식             에셋 위주
4  막기의 대가           조건부
5  적 전조               조건부
```
**1과 2가 80% 다.** 3은 그 위의 화장이고, 4·5 는 1단계 결과를 보고 정한다.
