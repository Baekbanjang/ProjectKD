...
## 🟢 2026-08-27 — 대상 탐색 필터 구조 + 자동 조준 결함 6건 + 적 상태 바 **(새 세션은 여기부터)**
...
```
0  빌드 확인     ⚠️ 디버그 표시 개선(KDLockOnComponent.cpp)은 **컴파일 안 된 상태**다.
                 승환이 빌드한 시점이 그 편집 이전 — 새 세션에서 반드시 빌드부터
1  MCP 재연결    unreal-engine 도구 23개가 끊겼다. 에디터 먼저 켜고 ● MCP :3000 확인
```

### 다음 (순서)
```
1  적 상태 바 칸 폭   엘리트 MaxPoise 25 = 250px > 바 폭 198. 안 3개는 dev-log 에
                  ⚠️ 위젯 실물을 MCP 로 먼저 확인할 것 — 구조 추정이 틀렸을 수 있다
...
4  캔슬 윈도우 02_02  20타 중 유일한 이상치(73%)
```
