# 지난 세션 기록 — 2026-07-30 ~ 2026-08-17

> 🗄️ **완료된 절 모음.** 현재 상태는 [`../CURRENT.md`](../CURRENT.md) 를 볼 것.
> 여기 적힌 "다음 할 일"은 **전부 이미 끝났거나 뒤집혔다.** 설계 근거·함정 기록으로만 읽는다.
> 2026-08-24 에 `2026-07-30-parallel-sessions.md`(1,052행)에서 분리했다.
...
**설계는 다 끝났다.** dev-log 에 SB 원본 규격(계층·치수·색)이 그대로 복원돼 있다. 만들기만 하면 된다.
...
색       Poise #EFFF74
소재     텍스처 0장 — ProgressBar + Border + 색상으로 시작
```

**Poise 를 먼저 하는 이유** = `Health` 는 데미지 계수가 0이라 안 줄어들지만 **`Poise` 는 `StaggerComponent` 가 실동작 중이라 지금 바로 움직인다.** 스태거·처형 타이밍이 눈에 보이는 첫 화면이 된다.

⚠️ **MCP 위젯 기능 한계 미검증** — `create_widget` / `add_widget_child` 액션은 있으나 앵커·바인딩·애니메이션까지 되는지 안 해봤다. **위젯 껍데기는 MCP, 바인딩·폴리싱은 에디터**가 현실적 그림. 먼저 15분 시험해볼 것.

### ✅ 총구 이펙트 — 배선 완료, 2차 PIE 미검증

```
NS_SB_Hit_Fire_Once   /Game/SB_Style_GameProject/Effect/   (작성자가 Flare 끄고 크기 절반)
배선   ANS_WeaponTrail  tag=Gun / socket=Muzzle / dur 0.10 / 발사 3프레임 전
대상   16개 몽타주 25발 / 트랙 Muzzle 신규
```

**★ GC 로는 총구에 못 붙인다** — 엔진이 `TargetCharacter->GetMesh()` 를 쓴다(`GameplayCueNotifyTypes.cpp:337`, UE5.6 실측). `ANS_WeaponTrail` 이 컴포넌트 태그로 무기 메시를 찾는 유일한 경로다.

**Niagara 파라미터 캐는 법** — Python 에 안 열린다. `grep -aoE "User\.[A-Za-z0-9_ ]+" 에셋.uasset`. ⚠️ 기본값은 이 방법으로 못 읽는다.

### HUD 결정사항

```
플레이어 바     HP + 탄약        ⚠️ 탄약 어트리뷰트가 없다 — 만들어야 함(.h = §0 승인)
적 표시         락온 대상만 상단중앙 (HP + Poise 2줄)
배선 방식       미결 (BP 구독 vs C++ 베이스 위젯)
소재            텍스처 0장으로 시작. 나중에 10×10 도트 한 장 얹으면 SB 질감
```

⚠️ **HUD 를 만들어도 두 바가 멈춰 있다** — `Health` 는 데미지 계수 0, `Ammo` 는 어트리뷰트 부재. 그래서 **Poise 부터** 간다.

### 알아둘 것

- **`docs/design/` 은 쓰기 권한 밖이다** — 설계 문서도 `docs/dev-logs/` 에 넣었다. 나중에 옮길 것
- **SB UI 텍스처를 빌드에 쓰면 저작권 문제.** 치수·색 실측과 구조 학습은 정당, 가져다 쓰기는 X. `sounds.spriters-resource.com` 도 동일(레퍼런스·임시용만)
- **Game UI Database** Inspector 가 색 헥스·폰트 크기를 뽑아준다. **봇 403 이라 사람이 직접** 봐야 함
- 힉스필드 **알파 지원 미확인** — 한 장 뽑으면 끝나는데 아직 안 해봤다

---

## ★★ 2026-08-16 — 판정 반경 측정 + 플레이어 정합. **PIE 통과·커밋 완료**

dev-log = `docs/dev-logs/2026-08-16-melee-radius-and-character-fit.md`
Content `2a526b3` `[Combat]` / `6637d5a` `[BP]` (로컬). **소스 변경 0. 워킹트리 클린**

### 결론 — 모션별 반경 차등은 근거가 없다

28개 창 전수 측정. 찌르기 0개(θ 44~84) / 날이 25개 전부 정면 중심선 통과(0.0~0.6cm) / 서브스텝 빈틈 0.
→ **`ANS_MeleeTrace` 개별 입력 안은 폐기. GA 기본값 하나 + 예외 소수로 확정.**

```
GA 5개        CapsuleRadius 3 -> 20   (Sweep 유지)
발차기 3개    foot_x -> foot_x (축 0, 지름 6cm 구)  =>  calf_x -> ball_x (51.8cm)
Combo_05_01   TipLine 오버라이드 해제 (TipLine = LineTrace = 굵기 0)
BP_SBPlayer   메시 Z -98 -> -95 (발이 3cm 파묻혀 있었다) / AirControl 0.05 -> 0.7
```

### ★ SB 본편 Eve 콜리전 원본 확보

**"덤프에 없다"가 아니라 `Art/Character` 추출 누락이었다.** 다시 받으니 콜리전 83개 + `CH_P_EVE_01_Collision.json`.
그동안 쓰던 Scarlet 환산은 폐기. 볼트 §9 전면 교체 / §9-B 로 내림.

- Eve `Collision_Weapon` **Thickness 30** → 우리 환산 **12.75~25.5 밴드**(반지름/지름 미확정). 반경 20이 밴드 안
- **발차기는 Calf 에 붙고 중심이 70cm 아래** — 우리 `calf→ball` 근거. 단 SB 보다 짧다
- **큰 공격은 두께가 아니라 형상을 Box 로 갈아탄다**(Big 100×40×40 / Heavy 150×60×60). 우리에겐 Box 모드 없음
- **SB melee 피격 = 부위별 Box 8개**(몸통 폭 24cm). 우리 캡슐 R34(지름 68)보다 **좁다** → **피격/이동 분리는 안 한다**

### 🟡 찌르기 재측정 — **적 작업으로 미룸 (2026-08-16 작성자 결정)**

**θ 를 루트모션 제거 좌표계에서 쟀는데 `02_Attack` 은 전부 루트모션이다.** 몸이 파고드는 찌르기는 전진량이 지워져 베기로 보인다.

작성자 판단 = **"공격이 적 타겟 소켓에 맞도록 조정하면 해소될 것"** → 적 작업 때 그 조정과 **한 묶음**으로 진행하고, 그때 걸리는 게 있으면 그 시점에 재측정한다.

근거가 있는 묶음이다 — `GetLockOnPoint` 의 `HalfHeight * 0.5` 갈래가 적마다 조준점을 튀게 하고, **적 DA 에 `LockOnSocketName` 을 채우면 그 갈래를 안 탄다**(2026-08-05 기록). 자동 조준·판정 도달거리가 같은 조준점을 공유하므로 찌르기 체감도 여기서 갈릴 수 있다.

### 남은 관측 (미착수)

- **도달거리 3건** — `02_03` 13cm(창이 감아올리는 구간) / `05_03` 0cm(물구나무라 칼이 지면 아래) / `Counter_L` 최적이 창보다 2프레임 앞
- **반경 20 과잉 여부** — 빗나갔는데 맞으면 15로. GA 5개 값 하나
- `AirControl 0.7` 은 **몽타주 밖 공중 구간에만** 먹는다(루트모션이 이동을 쥔다). 공중 콤보 중 방향 전환은 모션워핑 쪽 별개 작업
- 보류 = 이동 캡슐 Pawn Block→Ignore(적 AI 때) / 적 캡슐 R34→40 / `GravityScale` 플레이어 1.25 vs 적 1.0

---

## ★★ 2026-08-14 (2) — SB 판정 구조 조사 + 데이터 드리븐 검토. **코드 변경 0**

조사 결과 전문 = 볼트 `notes/Reference/StellarBlade_스킬테이블_3층구조.md` §9~11 (신규)

### 밝힌 것 3개

1. **SB 무기 판정 = 소켓 2개 + `Thickness`. 우리 `UAT_MeleeTrace`와 같은 구조**
   캐릭터가 `SBCharacterCollisionData` 에셋을 소유하고, 스텝이 그룹 이름으로 켠다(`bDefaultActive: false`)
   Eve 검 그룹 3종 = 기본 41 / `WeaponBig` 8(회전·다단) / `WeaponHeavy` 8(차지 참격). **기준은 모션 궤적 크기**
   **발차기는 발 크기로 안 때린다** — 실제 발 Box 8×12×8 인데 `Kick` 전용 Box 50×90×15 를 정강이에 따로 켠다
2. **`ShowData` = SB의 몽타주.** `Art/Show/.../*.json` 이 키 18개 타임라인이고 `SBShowTrailKey`(소켓 2개) · `SBShowSkillResultKey`(판정 트리거)가 우리 `ANS_WeaponTrail` · `ANS_MeleeTrace` 자리
   → **"SB는 테이블 / 우리는 몽타주"는 틀린 대조.** 진짜 차이는 타임라인에 **값을 적느냐 이름만 적느냐**
3. **자동 접근이 실재** — `CharacterMoveTable`의 `MovePositionType_Target` 655 / `TargetSocket` 25.
   단 일반 콤보는 `Self`(자기 앞 100cm)이고 **대시 어택만 타겟까지**. 회전(`bLookAtTarget`)과 접근은 별개

### 데이터 드리븐 검토 — 결론

선 = **스칼라냐 배열·참조냐** (표는 한 셀에 배열을 못 담는다)

| 대상 | 표로? | 언제 |
|---|---|---|
| **적 스탯** (순수 숫자 15개 × 적 N종) | ✅ | **적 작업 착수할 때.** 지금 DA 4개뿐이라 옮기는 일만 생긴다 |
| 판정 형상 | ❌ 보류 | `ANS_MeleeTrace` 28개 중 예외가 4개뿐. 예외 8개 넘으면 재검토 |
| 콤보 노드 | ❌ | `NextLinks` 배열 = 그래프라 표로 못 편다 |
| 무기 · 락온 설정 | ❌ | 행이 4개 / 1개. 표 이점 없음 |

⚠️ **"에셋 참조는 CSV 불가"는 내가 과장했던 것.** 경로 문자열로 넣을 수 있고, 문제는 **리네임에 안 따라간다**는 것뿐.

### 🔴 다음 세션 = 판정 반경 측정 (계획 확정, 미실행)

**MCP 필요.** 이 세션에서 unreal-engine MCP 가 끊겨 실행 못 함(포트 3000은 살아 있었으나 클라이언트가 놓침 → 되살리기 불가).

측정 목적 = **"모션마다 반경이 달라야 하나"를 숫자로 판정.** 그 결과로 구조가 갈린다
(크게 다르면 → 승환 안대로 `ANS_MeleeTrace` 개별 입력 / 비슷하면 → GA 기본값 + 예외 소수)

```
① 찌르기냐 휘두르기냐
   검 축 A = sword_weapon_r 본의 Z축 (팁 = 본 위치 + A × 129)
   팁 이동 M = 팁(f) - 팁(f-1)
   θ = acos(|A·M|)     작으면(0~30°) 찌르기 / 크면(60~90°) 휘두르기

② 찌르기에 필요한 반경
   검 축 선분과 캐릭터 정면 중심선의 최단거리 d
   필요 반경 = d - 34 (적 캡슐 R34)
   ※ 승환 체감 "정면 찔렀는데 적이 왼쪽이면 안 맞음" = d 가 37 넘는 구간

③ 휘두르기 반경 = max(날 반폭 8.5, 팁 프레임 갭 ÷ 2)

산출물 = ANS_MeleeTrace 28개 구간별 표 (θ / 유형 / d / 팁갭 / 권장 반경)
```

순서 = 에디터 켜고 `● MCP :3000` 확인 → Claude Code 실행 → 검 축이 Z인지 Idle로 재확인 → 28개 측정

### 측정 전이라도 확정된 값 (실측 근거 있음)

```
검 판정 축   Sword_Bottom(0,0,0) → Sword_Tip(0,-129,0) = 129cm
검날 단면    두께 4.5 × 폭 17 (메시 bounds)
발목→발볼    16.8cm
적 캡슐 R34 H88  /  플레이어 R34 H95
현재         GA 5개 전부 Sweep / CapsuleRadius 3.0
             ANS_MeleeTrace 28개 중 반경 오버라이드 0개, 모드 오버라이드 1개(05_01 TipLine)
             발차기 3개는 소켓이 foot_x→foot_x = 축 0 → 지름 6cm 구로 판정됨 ★결함
```

⚠️ **GA 기본값이 BP에 명시로 박힌 건지 코드 상속인지 미확인.** 그에 따라 고칠 곳이 BP 5개냐 `GA_MeleeTraceBase.h:57` 1곳이냐가 갈린다. MCP 붙으면 30초

---

## ★★ 2026-08-14 (1) — 공중 콤보 캔슬 윈도우 정렬. **PIE 통과·커밋 완료**
