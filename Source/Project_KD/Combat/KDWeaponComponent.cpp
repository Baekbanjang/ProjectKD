...
#include "Data/KDWeaponDataAsset.h"

UKDWeaponComponent::UKDWeaponComponent()
{
	PrimaryComponentTick.bCanEverTick = false;
}

void UKDWeaponComponent::AttachWeaponToHand()
{
	const FName HandSocket = CurrentWeapon ? CurrentWeapon->HandSocketName : AttachSocketName;
	AttachWeaponToSocket(HandSocket);
}

void UKDWeaponComponent::AttachWeaponToSheath()
{
...
void UKDWeaponComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UKDWeaponComponent::AttachWeaponToSocket(FName SocketName)
{
...
void UKDWeaponComponent::RegisterCombatTagListener()
{
	...
	if (!ASC)
	{
		...
		{
			World->GetTimerManager().SetTimerForNextTick(this, &UKDWeaponComponent::RegisterCombatTagListener);
		}
		return;
	}
	
	InCombatTagHandle = ASC->RegisterGameplayTagEvent(GameplayTags::State_Combat_InCombat, EGameplayTagEventType::NewOrRemoved)
		.AddUObject(this, &UKDWeaponComponent::OnInCombatTagChanged);
}

void UKDWeaponComponent::OnInCombatTagChanged(const FGameplayTag Tag, int32 NewCount)
{
...
UAnimMontage* UKDWeaponComponent::SelectEquipMontage(const FEquipMontageSet& Set) const
{
...
void UKDWeaponComponent::RegisterInActionTagListener()
{
	UAbilitySystemComponent* ASC = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(GetOwner());
	if (!ASC)
	{
		...
		if (UWorld* World = GetWorld())
		{
			World->GetTimerManager().SetTimerForNextTick(this, &UKDWeaponComponent::RegisterInActionTagListener);
		}
		return;
	}
	InActionTagHandle = ASC->RegisterGameplayTagEvent(GameplayTags::State_Combat_InAction, EGameplayTagEventType::NewOrRemoved)
		.AddUObject(this, &UKDWeaponComponent::OnInActionTagChanged);
}

void UKDWeaponComponent::OnInActionTagChanged(const FGameplayTag Tag, int32 NewCount)
{
	if (NewCount <= 0) return;
	AttachWeaponToHand();
}

void UKDWeaponComponent::BeginPlay()
{
	Super::BeginPlay();
