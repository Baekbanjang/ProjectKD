...
void UKDHitStopComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	OwnerASC->GenericGameplayEventCallbacks.FindOrAdd(GameplayTags::Event_Combat_HitStop)
		.AddUObject(this, &UKDHitStopComponent::OnHitStopEvent);
}

void UKDHitStopComponent::RequestHitStop(float Duration)
{
...
void UKDHitStopComponent::OnHitStopEvent(const FGameplayEventData* Payload)
{
	if (Payload)
	{
...
void UKDHitStopComponent::RestoreTime()
{
	// 정진 전 속도로 복귀
