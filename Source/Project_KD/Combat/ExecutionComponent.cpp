...
void UExecutionComponent::BeginPlay()
{
	...
	OwnerASC->GenericGameplayEventCallbacks.FindOrAdd(GameplayTags::Event_Combat_Hit)
		.AddUObject(this, &UExecutionComponent::OnHitReceived);
}

void UExecutionComponent::OnHitReceived(const FGameplayEventData* Payload)
{
...
bool UExecutionComponent::IsExecutionTrigger(const FGameplayTagContainer& InstigatorTags) const
{
	return !ExecutionTriggerTags.IsEmpty() && InstigatorTags.HasAny(ExecutionTriggerTags);
}

UAnimMontage* UExecutionComponent::GetExecutionMontage() const
{
...
void UExecutionComponent::HandleExecution()
{
	...
	GetWorld()->GetTimerManager().SetTimer(ExecutionTimer, this,
		&UExecutionComponent::FinishExecution, SafetyTime, false);
}

void UExecutionComponent::FinishExecution()
{
...
void UExecutionComponent::AbortForDeath()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().ClearTimer(ExecutionTimer);
