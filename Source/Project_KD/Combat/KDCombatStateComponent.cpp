...
UKDCombatStateComponent::UKDCombatStateComponent()
{
	PrimaryComponentTick.bCanEverTick = false;
}


void UKDCombatStateComponent::BeginPlay()
{
	Super::BeginPlay();

	GetWorld()->GetTimerManager().SetTimer(
		ScanTimerHandle, this, &UKDCombatStateComponent::ScanForEnemies, ScanInterval, true);

	
	RegisterInActionTagListener();
}

void UKDCombatStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	// 델리게이트 Attacking 종료
...
void UKDCombatStateComponent::ScanForEnemies()
{
...
bool UKDCombatStateComponent::HasEnemyInRange() const
{
...
// 공격 태그 델리게이트 등록
void UKDCombatStateComponent::RegisterInActionTagListener()
{
	UAbilitySystemComponent* ASC = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(GetOwner());
	if (!ASC)
	{
		...
		{
			World->GetTimerManager().SetTimerForNextTick(this, &UKDCombatStateComponent::RegisterInActionTagListener);
		}
		return;
	}

	InActionTagHandle = ASC->RegisterGameplayTagEvent(GameplayTags::State_Combat_InAction, EGameplayTagEventType::NewOrRemoved)
		.AddUObject(this, &UKDCombatStateComponent::OnInActionTagChanged);
}

// 공격 시작 시 즉시 전투
void UKDCombatStateComponent::OnInActionTagChanged(const FGameplayTag Tag, int32 NewCount)
{
