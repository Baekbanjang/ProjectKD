...
void UStaggerComponent::BeginPlay()
{
	...
	OwnerASC->GetGameplayAttributeValueChangeDelegate(UAS_CharacterBase::GetPoiseAttribute())
		.AddUObject(this, &UStaggerComponent::OnPoiseChanged);
}
...
void UStaggerComponent::OnPoiseChanged(const FOnAttributeChangeData& Data)
{
...
void UStaggerComponent::BeginStagger()
{
	...
	GetWorld()->GetTimerManager().SetTimer(StaggerTimeoutTimer, this,
		&UStaggerComponent::OnStaggerTimeout, StaggerDuration, false);

	// Pawn이 brain pause + movement freeze를 여기서 받음.
	OnStaggerBegin.Broadcast();
}

void UStaggerComponent::OnStaggerTimeout()
{
...
void UStaggerComponent::RecoverFromStagger()
{
...
void UStaggerComponent::HandleExecutionResolved(bool bSurvived)
{
...
void UStaggerComponent::AbortForDeath()
{
