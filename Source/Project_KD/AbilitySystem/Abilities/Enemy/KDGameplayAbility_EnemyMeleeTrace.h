...
#include "KDGameplayAbility_EnemyMeleeTrace.generated.h"

// 적 근접 공격 GA 베이스. 발동 시 플랜트(braking glide 제거) + 전조 큐.
// 자식은 소켓·몽타주·공격별 거동만 추가.
UCLASS(Abstract)
class PROJECT_KD_API UKDGameplayAbility_EnemyMeleeTrace : public UKDGameplayAbility_MeleeTrace
{
	GENERATED_BODY()

public:
	UKDGameplayAbility_EnemyMeleeTrace();
