...
#include "KDGameplayAbility_EnemyRushAttack.generated.h"

// 도끼 엘리트 gap-closer(돌진). 발동 시 Motion Warp target을 플레이어 쪽으로 설정 →
// 루트모션 몽타주가 그 지점까지 돌진. 단발 조준(리딩 없음) — 플레이어가 크게 움직이면 빗나감(회피 보상).
UCLASS()
class PROJECT_KD_API UKDGameplayAbility_EnemyRushAttack : public UKDGameplayAbility_EnemyMeleeTrace
{
	GENERATED_BODY()
