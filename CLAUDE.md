...
- **세션 종료 전**: 브릿지(`notes\_세션브릿지.md`)의 `## ✅ 처리됨` 에 **자기가 한 것만** 옮긴다. 상대 구역 항목은 근거만 덧붙이고 옮기지 않는다
- **기능 완료 시**: `docs/dev-logs/{날짜}-{기능명}.md` 자동 생성(구현 클래스·주요 값·아키텍처 결정·검증 결과) + `docs/INDEX.md` 한 줄 등록 (2026-08-25 전역에서 이사)
- **세션 마감** (2026-09-04 신설): Stop 훅이 마감을 감지하면 **묻지 말고 `wrap-up` 스킬을 실행한다.** 문서 갱신(dev-log · INDEX · CURRENT · 브릿지)까지는 승인 불필요. **커밋·푸시는 반드시 승인받는다.** 위 두 항목(세션 종료 전 · 기능 완료 시)이 그 루틴 안에 들어간다
...
| ★★ `C:\Users\asdasd\Desktop\Obsidian_organize\ProjectKD\notes\_세션브릿지.md` | **클라우디안(볼트) 세션과의 역할 구분 · 파일별 편집 담당 · 대기 항목.** 두 세션 동시 작업 시 유일한 통신 수단 (실시간 통신 불가, 파일 게시판 방식) |

- **`docs/archive/` 는 읽지 않는다** — 폐기된 버터/길동 라인. 여길 현역으로 읽고 설계하는 게 사고의 주원인
- ~~기획 참조가 필요하면 `docs/design/기획/`~~ → 🗑️ **2026-08-24 삭제됨.** 볼트 `notes/ProjectKD/기획/` 에 최신본 보존(구세대 동결). **스토리·세계관은 무효(길동 기반), 시스템 설계 문법만 유효**

#### 1-1) 두 세션 역할 구분 ★ (2026-08-24 신설)

> **코드 · 에셋 · 빌드 = 이 세션(KD) / 문서 · 지식 · 취업 = 클라우디안(볼트) 세션**

| 만지는 것                                                                                           | 담당                                                       |
| --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| `Source\**` `Content\**` `Config\**` `*.uproject`                                                   | ✅ **이 세션**                                             |
| `docs\dev-logs\` `docs\handoffs\` `docs\archive\`                                                   | ✅ **이 세션**                                             |
| `docs\INDEX.md` `docs\PROJECT_OVERVIEW.md`                                                          | ✅ **이 세션** (dev-log 등록이 최빈 변경 — 34회 중 대부분) |
| 볼트 `notes\**` 전부 (특히 `notes\코드구조\`)                                                       | ❌ **볼트 세션** — 읽기만, 수정은 브릿지로 요청            |
| `Desktop\SeungHwan\개인\취업\**`                                                                    | ❌ **볼트 세션** (자소서·이력서·포폴)                      |
| `notes\_세션브릿지.md`                                                                              | 🤝 **양쪽** (절 단위로 나눠 씀)                            |
| `docs\design\` `docs\specs\` `docs\reference\` · 본 `CLAUDE.md` · 볼트 `CLAUDE.md` · `~\.claude\**` | 🔒 **승환만** (전역 settings 잠금)                         |

🔴 **판정 권한 원칙**: 자기 눈으로 원본을 볼 수 있는 쪽만 단정한다. 못 보는 쪽은 **"확인해줘"라고 요청**한다.

- **이 세션만 판정 가능**: C++ 코드 유무 · `.uasset`/`.umap` 상태 · 에셋 참조 여부 · 빌드·PIE 결과 · Content 커밋 판단
- **볼트 세션만 판정 가능**: 노트 내용 · 위키링크·고아 판정 · 문서 교차대조 · 볼트 git 상태 · 취업 서류

⚠️ **`notes\코드구조\` 는 볼트 세션이 쓴다.** 이 세션은 dev-log 를 남기고, 볼트 세션이 그것을 읽어 반영한다. 직접 고치지 않는다.

⚠️ **볼트 세션이 문서를 지우거나 옮기면** 브릿지에 "INDEX 반영 요청"이 올라온다. 그 실행은 이 세션이 한다.

**2026-08-24 위반 3건** (왜 이 규칙이 필요한가)

1. 볼트 세션이 Content 에셋 상태를 판정 → `Kildong` 대소문자 실수로 "에셋 없음" 오판, 이어서 구버전 `ABP_Player` 보고 "현역" 재오판
2. 볼트 세션이 "바이너리는 못 본다"며 포기 → 이 세션이 크기 −44 서명으로 뚫었다. **포기 대신 넘겨야 했다**
3. 이 세션이 볼트 git 상태를 브릿지에 기재 → 옛 정보("초기 커밋 필요")를 이어받음. 실제로는 커밋하면 안 되는 저장소
...
- **새 클래스 파일 생성 전** 파일 목록(경로 + 클래스명 + 상속 관계)을 먼저 보여주고 승인받는다 (2026-08-25 전역에서 이사)
...
| `wrap-up`         | 세션 마감 · Stop 훅 감지 시     | dev-log·INDEX·CURRENT·브릿지 6단계. 하청 경계 명문화(브릿지·CURRENT = 메인 전용) |
...
- **대량 조회(목록·집계·분류·수치) = `execute_python`** — AssetRegistry로 돌리고 가공 결과만 회수. `search_assets`는 단건 확인만(패턴 무시 + 100개 통째 반환이라 목록용 금지) / 재귀 `list`는 트랜스포트 끊김 / 에디터 상태 확인도 스크린샷 대신 Python 조회 (2026-08-25 전역에서 이사)

### 모델 라우팅 — Fable 5 세션 (2026-08-25 신설)

- Fable을 메인으로 켠 세션 = **도구 작업 전부 하청** — 조사·파일 스캔·구현(편집 허가 시)·빌드·MCP 대량 조회는 `sonnet`(복잡한 구현만 `opus`) 서브에이전트로. Fable 본체 = 설계·진단·판정·하청 결과 종합·diff 검수만
- 이유 = Fable은 한도 2배 소모 (2026-08-25 리팩토링 세션에서 조사 4하청 + 실행 3하청 구조로 실증)

> `.h` / `.cpp` = 작성자 입력 영역. **agent도 코드 파일 수정 금지** — 설계·코드블록 반환까지만 (§0)

### 하청 기본값 — 조사·문서는 백그라운드 서브에이전트 (2026-08-27 신설)

메인 세션에서 도구 루프를 길게 돌리면 **그동안 승환이 세션을 못 쓴다.** 한도 절약보다 이게 먼저다.

| 하청(백그라운드)                            | 메인 직접                             |
| ------------------------------------------- | ------------------------------------- |
| 여러 파일 조사 · 덤프 뒤지기 · 전수 스캔    | 판정 · 설계 · 결론 종합               |
| MD 장문 작성·갱신 (dev-log 초안, 볼트 노트) | 한두 번의 확인 조회 · 짧은 기록 한 줄 |

- 회수 형식 = **결론 3~5줄 + 근거 경로.** 하청 산출물을 대화에 통째로 붙이지 않는다
- ⛔ **하청 금지 3종** — ①판정(근거만 받고 결론은 메인) ②`.h`/`.cpp` 편집(§0 동일 적용)
  ③브릿지·`CURRENT.md`(그날 맥락을 아는 메인이 쓴다. 하청은 실측표까지만)
